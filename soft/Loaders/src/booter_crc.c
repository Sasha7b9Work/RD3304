/*! \file booter_crc.c
 *
 * \brief Расчет контрольной суммы по алгоритму CRC32
 * 
 * \page page_crc32 Расчет контрольной суммы по алгоритму CRC32.
 *
 * Алгоритм подсчета контрольной суммы использует полином 0xEDB88320
 *
 */
 
 /*! \defgroup group_crc32 Модуль расчета CRC32
  *
  * @{
 */
 ...
 /*! \brief Таблица для расчета CRC32 с полиномом 0xEDB88320 
  *
  * Таблица для расчета CRC32 с полиномом 0xEDB88320. Таблица посчитана заранее для ускорения работы программы 
  * и размещена в ПЗУ.
 */
 /*
...
void Init_CRC32( void )
{
 ...
 /*! Расчет CRC32 с полиномом 0xEDB88320
  *
  * \param pData указатель на данные для расчета
  * \param nLen  количество байт для расчета
  * \param crc   текущее значение CRC
  * \return CRC32
 */
 uint32_t ProcessCRC( uint8_t* pData, int nLen, uint32_t crc )
 {
   register uint8_t* pdata = pData;
   while (nLen--)
     {
      crc = CrcTable[(unsigned char)(crc) ^ *pdata++] ^ (crc >> 8);
     }
   return crc;
 }

 /*! Шаг расчета  CRC32 с полиномом 0xEDB88320 
  *
  * \param b байт для расчета
  * \param crc текущее значение CRC
  * \return CRC32
 */
 /*
 ...
 {
   crc = crc_table[ (unsigned char)(crc) ^ b ] ^ (crc >> 8);
   return crc;
 }*/
#endif
/*! @} */
